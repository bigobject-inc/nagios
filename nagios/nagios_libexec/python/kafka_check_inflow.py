...
# python ...py $hostname $kafka_host $kafka_port $topic $expire_min
def main():
    # verify input
    args = sys.argv
    if len(args)<6:
    ...
    # verify existent of topic
    if kafka_topic not in kf_consumer.topics():
        print("topic: {!s} does not exist".format(kafka_topic))
        sys.exit(NAGIOS_WARNING)
    ...
    for partnum, offset in partnum_offset.items():
        ...
        # query record in DB
        row = record.get(str(partnum), None)
        if row is None:
